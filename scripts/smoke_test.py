#!/usr/bin/env python3
"""Exercise every registered tool against the live Cloud Logging API and report what is broken.
...
Read-only by construction: every tool on this server only reads Cloud Logging,
and the report prints only tool names, statuses and row counts — never
payloads, which would otherwise carry WAF log data into logs.

Usage:
    # In-process: imports the server, uses the configured API credentials.
    CLOUDARMOR_PROJECT=... GOOGLE_APPLICATION_CREDENTIALS=... \
        uv run python scripts/smoke_test.py

    # Iterate on one spec while writing it.
    uv run python scripts/smoke_test.py --only enforce --traceback
...
async def main_async(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        results = await run_probes(
            ...
            # WAF log data must not reach the report: Cloud Logging errors can quote the
            # filter (project, backend names) they were asked about. --traceback still shows the full
